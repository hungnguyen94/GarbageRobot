...
//    transA(eyeShapeLeft[i]);
//    transB(eyeShapeRight[i]);
    delayMicroseconds(700);
    if (i >= 7) {
      trans(eyeShapeLeft[i],eyeShapeRight[0]);
//      transA(eyeShapeLeft[0]);
//      transB(eyeShapeRight[0]);
      delayMicroseconds(10);
    }
    else {
      trans(eyeShapeLeft[i],eyeShapeRight[i+1]);
//      transA(eyeShapeLeft[i + 1]);
//      transB(eyeShapeRight[i + 1]);
      delayMicroseconds(10);
    }
    clock_on();
  }
...
void trans(int left, int right){
  ...
  Wire.beginTransmission(0x20);
  Wire.write(0x12);
  Wire.write(left); // set all of port A to outputs
  Wire.endTransmission();
  
  Wire.beginTransmission(0x20);
  Wire.write(0x13);
  Wire.write(right); // set all of port B to outputs
  Wire.endTransmission();
